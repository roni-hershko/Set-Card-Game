...
public class UserInterfaceSwing extends JFrame implements UserInterface {
    ...
    public UserInterfaceSwing(Logger logger, Config config, Player[] players) {
        ...
        setTitle("C_All_Rights_Reserved - Avinoam David");
    ...
    private class GamePanel extends JLayeredPane {
        ...
        private final Image[][] keyBoard_keys;
        private final Image[][] onGameKeys;
        ...
        private GamePanel() {
            ...
            keyBoard_keys = new Image[config.rows][config.columns];
            onGameKeys = new Image[config.rows][config.columns];
            ...
            for (int row = 0; row < config.rows; row++) {
                for (int column = 0; column < config.columns; column++) {
                    keyBoard_keys[row][column] = loadImageResource("cards/" + (3000 + i) + ".png");
                    onGameKeys[row][column] = loadImageResource("cards/" + (3000 + i) + ".png");
                    i++;
                }
            }
        ...
        private void placeCard(int slot, int card) {
            ...
            keyBoard_keys[row][column] = onGameKeys[row][column];
            validate();
            repaint();
        }
        ...
        public void paintComponent(Graphics g) {
            ...
            for (int row = 0; row < config.rows; row++) {
                for (int column = 0; column < config.columns; column++) {
                    ...
                    g.drawImage(keyBoard_keys[row][column], (column * config.cellWidth) , (row * config.cellHeight) , this);
                    i++;
                }
            }
        }
