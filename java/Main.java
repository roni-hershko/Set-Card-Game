...
public class Main {
    ...
    static Boolean settingsDone;
    static Object settingsKey;
    ...
    public static void makeSettings (Config config) {
        SettingsPanel settings = new SettingsPanel(config, settingsKey);
        try {
            SwingUtilities.invokeAndWait(settings);
            if (!settingsDone) {
                synchronized (settingsKey) {settingsKey.wait();}
            }
